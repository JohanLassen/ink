...
We peak call the training data and validation data simultaneously to ensure both datasets have the same peaks
...
## Visualize the raw data before normalization
```{r umap raw data visualization, fig.align='center'}
...
## Define data (again) for the analysis
```{r}
pen       <- paste0("pen", inkdata2 |> filter(type == "sample") |> pull(pen))
age       <- inkdata2 |> filter(type == "sample") |> pull(age)
x         <- inkdata2 |> filter(type == "sample") |> select(starts_with("M"))
